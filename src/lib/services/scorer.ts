...
Personality Traits: ${applicant.personalityTraits.join(', ')}`
    ...
    const rolesText = roles.map((role, index) => {
      let roleText = `${role.name} (Position ${index + 1})`;
      if (role.description) {
    ...
    return `You are an expert team formation analyst. Your job is to analyze each applicant and score how well they fit each role based on their specific background and the role's requirements.
...
1. You MUST analyze each applicant's occupation, years of experience, skills, and personality traits
2. You MUST match them against each role's specific description and requirements
3. Score each applicant (1-${teamSize}) against each role (1-${rolesCount}) on a scale of 0-100
4. Give HIGH scores (90-100) when there's a clear match between applicant and role
5. Give LOW scores (0-30) when there's a clear mismatch
6. Be logical: A cheerleader should score high for Cheerleader role, a logistics manager should score high for Logistics Manager roles
7. Consider years of experience: More experience in relevant fields should increase scores for related roles

ANALYSIS PROCESS:
For each applicant, think: "Does their occupation/experience/skills/personality match what this role needs?"
- If YES: Score 80-100
- If SOMEWHAT: Score 40-79  
- If NO: Score 0-39

REQUIRED OUTPUT FORMAT:
Respond with ONLY a ${teamSize}x${rolesCount} JSON array (no other text):
- Row 1 = Applicant 1's scores for all ${rolesCount} roles
- Row 2 = Applicant 2's scores for all ${rolesCount} roles
${teamSize > 2 ? '- Row 3+ = Additional applicants...' : ''}
- Each score must be 0-100

DO NOT copy any example patterns. Analyze each person individually based on their actual profile.
